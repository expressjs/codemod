...
const parentExpressionType = [
  "ArrowFunctionExpression",
  "FunctionExpression",
] as const;

const unifiedMagicString = (
  ...
  if (
    ...
    path.value.arguments = [
      logicalExpression(
        "||",
        callExpression(
          memberExpression(identifier(projectRequestName), identifier("get")),
          [identifier('"Referrer"')]
        ),
        literal("/")
      ),
...
const recursiveParent = (parent: ASTPath<ASTNode>): string | null => {
  if (parentExpressionType.some((type) => parent.value.type === type)) {
    const foundNode = parent.value as unknown as
      | ArrowFunctionExpression
      | FunctionExpression;
...
export default function transformer(file: FileInfo, _api: API) {
  ...
  parsedFile
    ...
    .map((path) =>
      unifiedMagicString(path, recursiveParent(path.parentPath) || "req")
    );
  parsedFile
    .find(CallExpression, {
      callee: {
